...
#include <cmath>
using namespace std;
int dselect(int* a, int lo, const int hi, const int n, int& partitionStage);
...
int pivot(int* a, int lo, int hi, int &partitionStage){
    ...
    return dselect(a, lo, lo + new_hi, lo + (hi - lo)/10, partitionStage);
}
...
int dselect(int* a, int lo, const int hi, const int n, int& partitionStage) {
    ...
    if (hi == lo) return lo;
    int pindex = pivot(a, lo, hi, partitionStage);
    pindex = partition(a, lo, hi, pindex);
    partitionStage += 1;
    if(n == pindex) return n;
    else if (n < pindex){
        return dselect(a, lo, pindex - 1, n, partitionStage);
    }
    else {
        return dselect(a, pindex + 1, hi, n, partitionStage);
    }
}
...
int main(int argc, char* argv[]) {
    ...
    int kth = dselect(a, 0, size - 1, n - 1, partitionStage);
    cout<<partitionStage<<endl;
